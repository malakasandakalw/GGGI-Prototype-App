  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-6 py-16">
      <div className="mb-10 flex flex-col items-center">
        ...
        <p className="text-gray-400 text-sm">Select a user role to explore its flow</p>
      </div>
      <div className="flex flex-col gap-3 w-full max-w-md">
        {roles.map((role) => (
          <Link key={role.href} href={role.href}>
            <button className="w-full bg-white hover:bg-gray-900 hover:text-white border border-gray-200 text-gray-900 rounded-2xl px-6 py-5 text-left transition-all shadow-sm hover:shadow-md group">
              <span className="block text-base font-semibold">{role.label}</span>
              <span className="block text-xs mt-0.5 text-gray-400 group-hover:text-gray-300">{role.description}</span>
            </button>
          </Link>
        ))}
      </div>
