        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-50">
              ...
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Amount (LKR)</th>
          ...
          <tbody>
            {fees.map((f) => (
              <tr key={f.id} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-6 py-4 text-gray-500">{f.due}</td>
                ...
                <td className="px-6 py-4 text-right">
                  {f.status === "Unpaid" && (
                    <button onClick={() => setPayingFee(f)} className="text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg transition-colors">Pay Now</button>
                  )}
                </td>
              </tr>
            ))}
      ...
      {payingFee && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md shadow-2xl relative">
            ...
            <p className="text-sm text-gray-400 mb-5">{payingFee.description}</p>

            <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-4 mb-5 flex justify-between items-center">
              <span className="text-sm text-indigo-600 font-medium">Amount Due</span>
              <span className="text-2xl font-bold text-indigo-700">LKR {payingFee.amount.toLocaleString()}</span>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {["Credit / Debit Card", "Bank Transfer", "Online Banking"].map((m) => (
                    <button key={m} onClick={() => setMethod(m)} className={`text-xs px-3 py-2.5 rounded-xl border transition-all text-center ${method === m ? "border-indigo-500 bg-indigo-50 text-indigo-700 font-medium" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>
                      {m}
                    </button>
                  ))}
                </div>
              ...
              {method === "Credit / Debit Card" && (
                <div className="space-y-3">
                  <div>
                    ...
                    <input placeholder="1234 5678 9012 3456" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-indigo-300" />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">Expiry</label>
                      <input placeholder="MM / YY" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-indigo-300" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-gray-700 mb-1.5">CVV</label>
                      <input placeholder="•••" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:border-indigo-300" />
                    </div>
                  </div>
                </div>
              )}
              ...
              {method === "Bank Transfer" && (
                <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-1.5">
                  <div className="flex justify-between"><span className="text-gray-400">Bank</span><span className="font-medium">People's Bank</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Account No.</span><span className="font-mono font-medium">123-4567-8901</span></div>
                  <div className="flex justify-between"><span className="text-gray-400">Reference</span><span className="font-mono font-medium text-indigo-600">S2024001</span></div>
                </div>
              )}
            </div>

            <div className="flex gap-3 mt-5">
              <button onClick={handlePay} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-2.5 text-sm font-medium transition-colors">Confirm Payment</button>
