    ...
    <div className="w-full space-y-6">
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
          ...
          <span className="text-xs bg-gray-100 text-gray-500 px-2.5 py-1 rounded-full">{enrolled.length} courses</span>
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-50">
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Course</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Lecturer</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Schedule</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Room</th>
              <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Credits</th>
            ...
            {enrolled.map((c) => (
              <tr key={c.code} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                <td className="px-6 py-4">
                  ...
                  <p className="text-xs text-indigo-500 font-mono mt-0.5">{c.code}</p>
                </td>
      ...
      {available.length > 0 && (
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-gray-50 flex items-center justify-between">
            ...
            <span className="text-xs bg-indigo-50 text-indigo-600 px-2.5 py-1 rounded-full font-medium">Registration open</span>
          </div>
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-50">
                ...
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Schedule</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Credits</th>
                <th className="text-left px-6 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wide">Seats</th>
                <th className="px-6 py-3" />
              </tr>
            </thead>
            <tbody>
              {available.map((c) => (
                <tr key={c.code} className="border-b border-gray-50 last:border-0 hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => setConfirmCourse(c)} className="flex items-center gap-1.5 text-xs bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-lg transition-colors ml-auto">
                      <Plus size={12} /> Enroll
                    </button>
                  </td>
                </tr>
      ...
      {confirmCourse && (
        <Modal onClose={() => setConfirmCourse(null)}>
          ...
          <p className="text-sm text-gray-400 mb-5">Review details before enrolling</p>
          ...
          <div className="flex gap-3">
            <button onClick={handleEnroll} className="flex-1 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl py-2.5 text-sm font-medium transition-colors">Confirm Enrollment</button>
      ...
      {dropCourse && (
        <Modal onClose={() => setDropCourse(null)}>
          ...
          <p className="text-sm text-gray-400 mb-5">This action cannot be undone during the semester.</p>
