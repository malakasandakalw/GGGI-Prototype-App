...
const courseColors: Record<string, string> = {
  CS201: "bg-indigo-50 border-l-4 border-indigo-400",
...
export default function Timetable() {
  return (
    <div className="w-full space-y-6">
      {/* Legend */}
      <div className="bg-white rounded-2xl border border-gray-100 p-4 shadow-sm flex items-center gap-6 flex-wrap">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-100 bg-gray-50">
              <th className="text-left px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wide w-32">Time</th>
              {days.map((d) => (
                <th key={d} className="text-left px-5 py-3.5 text-xs font-semibold text-gray-400 uppercase tracking-wide">{d}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {slots.map((slot) => (
              <tr key={slot.time} className="border-b border-gray-50 last:border-0">
                <td className="px-5 py-4 text-xs text-gray-400 font-mono whitespace-nowrap">{slot.time}</td>
                {days.map((d) => {
                    ...
                    <td key={d} className="px-3 py-3">
                      {entry ? (
                        <div className={`rounded-lg px-3 py-2.5 ${courseColors[entry.code] ?? "bg-gray-50 border-l-4 border-gray-300"}`}>
                          ...
                          <p className="text-xs text-gray-500 font-mono mt-0.5">{entry.code} · {entry.room}</p>
                        </div>
                      ) : (
                        <div className="h-12" />
                      )}
