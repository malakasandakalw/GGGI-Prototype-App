...
export default function HODCalendar() {
  ...
  function save() {
    addCalendarEvent({ title, date, type, moduleId, venue, notes: instr });
    toast.success("Event added");
    setOpen(false); setTitle(""); setVenue(""); setInstr("");
  }
          ...
          <div className="space-y-4">
            <div className="space-y-1.5"><Label className="text-xs">Event Type</Label>
              <Select value={type} onValueChange={(v) => setType(v as CalendarEventType)}>
                ...
                <SelectContent>{["exam", "deadline", "quiz", "semester", "holiday"].map((t) => <SelectItem key={t} value={t} className="capitalize">{t}</SelectItem>)}</SelectContent>
              </Select>
            </div>
